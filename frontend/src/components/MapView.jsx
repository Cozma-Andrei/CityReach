...
export function MapView() {
  ...
  const { mapRef, addGeoJsonLayer, goToBbox, updateTransportFilters, updateAdminLevelFilters, setupNeighborhoodClickFilter, showAccessibilityHeatmap, searchFeature } = useGeoMap({
      ...
      <div className="hero">
        ...
        <div style={{ marginLeft: "auto" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            {userRole === "admin" && (
              <Link to="/admin" style={{ color: "#8fb5ff", textDecoration: "none", fontWeight: "600", padding: "8px 14px", background: "#1f2c47", borderRadius: "8px" }}>
                Admin
              </Link>
            )}
      ...
      <div className="grid">
        <section className="panel">
          <div className="flex-between">
            <h3>Search</h3>
            <span className="badge">Find station/neighborhood</span>
          </div>
          <div className="controls" style={{ display: "flex", gap: "12px", alignItems: "flex-end", marginBottom: "24px", flexWrap: "nowrap" }}>
            <div className="field" style={{ flex: "1 1 auto" }}>
              <label>Search by name</label>
              <input
                placeholder="Enter station or neighborhood name..."
                value={searchQuery}
                ...
                onKeyDown={async (e) => {
                  if (e.key === "Enter" && searchQuery.trim()) {
                    try {
                      setStatus("Searching...");
                      const result = await searchFeature(searchQuery, searchType, setStatus);
                    } catch (err) {
                      console.error("Search error:", err);
                      setStatus("Error searching. Make sure you have loaded stations/neighborhoods.");
                    }
                  }
                }}
              />
            ...
            <button
              onClick={async () => {
                ...
                try {
                  setStatus("Searching...");
                  await searchFeature(searchQuery, searchType, setStatus);
                } catch (err) {
                  console.error("Search error:", err);
                  setStatus("Error searching. Make sure you have loaded stations/neighborhoods.");
                }
              }}
              style={{ flex: "0 0 auto", marginBottom: "0", whiteSpace: "nowrap", padding: "10px 16px", height: "fit-content" }}
            >
              Search
            </button>
          </div>
